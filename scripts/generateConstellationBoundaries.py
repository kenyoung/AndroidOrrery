#!/usr/bin/env python3
"""
Generate constellation_boundaries.bin from Roman (1987) constellation boundary data.

Source: CDS VI/42 (https://cdsarc.cds.unistra.fr/ftp/VI/42/data.dat)
Format: Each record defines a rectangular strip in B1875.0 equatorial coordinates.
        Columns: RA_low (hours), RA_high (hours), Dec_low (degrees), constellation abbreviation.
        Records are sorted by decreasing Dec_low.

Binary output format:
    Each record = 13 bytes:
        RAlow   (float32, little-endian, hours)
        RAhigh  (float32, little-endian, hours)
        Declow  (float32, little-endian, degrees)
        index   (uint8, constellation index 0-87)
    Records are sorted by decreasing Declow (same as source).
    Total: 357 records * 13 bytes = 4641 bytes.

Constellation indices use alphabetical ordering by abbreviation, matching the
constellationNames array in ConstellationBoundary.kt.
"""
...
# Roman (1987) VI/42 constellation boundary data — B1875.0 equatorial coordinates.
# 357 rectangular strips sorted by decreasing declination.
BOUNDARY_DATA = """\
...
# 88 IAU constellation abbreviations, sorted alphabetically.
# Indices 0-87 match the constellationNames array in ConstellationBoundary.kt.
ABBREV_SORTED = [
...
abbrevToIndex = {abbr: i for i, abbr in enumerate(ABBREV_SORTED)}


def main():
    records = []
    for line in BOUNDARY_DATA.strip().splitlines():
        parts = line.split()
        raLow = float(parts[0])
        raHigh = float(parts[1])
        decLow = float(parts[2])
        abbrev = parts[3]
        idx = abbrevToIndex[abbrev]
        records.append((raLow, raHigh, decLow, idx))

    # Verify sorted by decreasing decLow (source data is already in this order)
    for i in range(1, len(records)):
        assert records[i][2] <= records[i - 1][2], \
            f"Record {i} not sorted: {records[i][2]} > {records[i-1][2]}"

    outPath = os.path.join(ASSETS_DIR, 'constellation_boundaries.bin')
    os.makedirs(ASSETS_DIR, exist_ok=True)
    with open(outPath, 'wb') as f:
        for raLow, raHigh, decLow, idx in records:
            f.write(struct.pack('<fff', raLow, raHigh, decLow))
            f.write(struct.pack('B', idx))

    print(f"Wrote {len(records)} records ({os.path.getsize(outPath)} bytes) to {outPath}")
